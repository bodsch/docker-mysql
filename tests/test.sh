...
if [[ -z "${MYSQL_SYSTEM_USER}" ]] || [[ -z "${MYSQL_ROOT_PASS}" ]]
then
  echo "run 'make compose-file' first"
  exit 1
fi
...
DBA_OPTS="--host=127.0.0.1 --user=${MYSQL_SYSTEM_USER} --password=${MYSQL_ROOT_PASS} --port=${MYSQL_PORT} --batch --skip-column-names"
...
wait_for_database() {
  ...
  until [[ ${RETRY} -le 0 ]]
  do
    timeout 1 bash -c "cat < /dev/null > /dev/tcp/127.0.0.1/${MYSQL_PORT}" 2> /dev/null
    if [ $? -eq 0 ]
    then
      break
    else
  ...
  if [[ $RETRY -le 0 ]]
  then
    echo "could not connect to the database instance '${ICINGA2_MASTER}'"
    exit 1
  fi
